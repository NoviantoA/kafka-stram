...
public class CommodityFourStream {
    ...
    public KStream<String, OrderMessage> kStreamCommodityTrading(StreamsBuilder builder){
        ...
        maskedCreditCardStream.mapValues(CommodityStreamUtil::mapToOrderPattern).split()
                .branch(CommodityStreamUtil.isPlastic(), Branched.withConsumer(kStream -> kStream.to("t-commodity-pattern-two-plastic", Produced.with(stringSerde, orderPatternSerde))))
                .branch((k, v) -> true, Branched.withConsumer(kStream -> kStream.to("t-commodity-pattern-two-notplastic", Produced.with(stringSerde, orderPatternSerde))));
